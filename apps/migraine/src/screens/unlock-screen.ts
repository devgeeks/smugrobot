  ...
  if (err instanceof EchidnaJsError) {
    switch (err.code) {
      ...
      case "TAMPERED":
        return {
          message:
            "Your data appears to be corrupted, not a passphrase problem — retrying won't help. Restore from a backup if you have one.",
          wrongPassphrase: false,
        };
      case "NEEDS_MIGRATION":
        return {
          message: "Your data needs to finish an internal upgrade. Reload the page and try again.",
          wrongPassphrase: false,
        };
...
export function mountUnlockScreen(root: HTMLElement): () => void {
  ...
  wrap.innerHTML = `
    <div class="unlock-card-wrap">
      <vault-card border class="unlock-card">
        <div class="unlock-logo">Migraines</div>
        <h1 class="unlock-heading">${creating ? "Create a master passphrase" : "Enter your passphrase"}</h1>
        <p class="unlock-sub">${
          creating
            ? "Your migraine log will be encrypted with this passphrase. Don't lose it — it cannot be recovered."
            : "Enter your passphrase to unlock your migraine log."
        }</p>
      ...
      <div id="kdf-progress" class="kdf-progress">
        <span class="kdf-progress-label" aria-live="polite">Unlocking your migraine log…</span>
  ...
  const doUnlock = async () => {
    ...
    lastAnnouncedPct = -1;
    updateProgress(0, creating ? "Encrypting your vault…" : "Unlocking your migraine log…");
