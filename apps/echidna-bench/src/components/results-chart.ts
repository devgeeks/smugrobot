...
export class ResultsChart {
  ...
  render(results: BenchResult[]): void {
    ...
    const maxThroughput = Math.max(
      1,
      ...results.flatMap((r) => [r.encrypt.throughputMBs, r.decrypt.throughputMBs].filter(Number.isFinite)),
    )
  ...
  private appendBar(
    svg: SVGSVGElement,
    opts: { x: number; baselineY: number; plotHeight: number; yMax: number; value: number; color: string; tooltip: string },
  ): void {
    const ns = 'http://www.w3.org/2000/svg'
