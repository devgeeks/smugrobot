...
export async function mountAppScreen(root: HTMLElement): Promise<void> {
  ...
  function setMobilePane(
    ...
    if (pane === "folders") {
      ...
      headerBackBtn.setAttribute(
        "aria-label",
        pane === "list" ? "Back to folders" : "Back to notes",
      );
    }
  ...
  unsub = subscribe(async () => {
    ...
    try {
      ...
      if (state.selectedNoteId !== null && state.selectedNoteId !== editorPane.currentNoteId) {
...
export async function loadFolders(): Promise<void> {
  ...
  const folders = all.filter((m) => m["type"] === "folder") as ReturnType<
    typeof getState
  >["folders"];
