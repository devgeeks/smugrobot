...
function focusVaultButton(el: Element): void {
  (el as HTMLElement & { shadowRoot: ShadowRoot | null }).shadowRoot?.querySelector('button')?.focus()
}

function focusVaultInput(el: Element): void {
  (el as HTMLElement & { shadowRoot: ShadowRoot | null }).shadowRoot?.querySelector('input')?.focus()
}
...
export function promptDialog(opts: {
  ...
  return new Promise((resolve) => {
    ...
    const overlay = createOverlay(titleId, opts.title, bodyHtml, opts.confirmLabel, opts.cancelLabel ?? 'Cancel', false)
    document.body.appendChild(overlay)
