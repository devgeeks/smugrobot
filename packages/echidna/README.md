...
| `INVALID_KDF_PARAMS` | KDF params (read from `vault/kdf`) are malformed or out of bounds                                       |
...
- **Version byte is unauthenticated:** it sits outside the MAC, so a compromised backend can flip a `0x02` body to `0x01`, denying reads (`NEEDS_MIGRATION`) until re-migrated; `migrate()` detects the flip and repairs the body in place instead of laundering it into a corrupted, MAC-valid `0x02` blob.
