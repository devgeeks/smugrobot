    ...
    async get(key: string): Promise<Uint8Array | null> {
      try {
        ...
        if (!attachment || typeof attachment.data !== "string") {
          throw new Error(`pouchDbAdapter: doc "${key}" is missing its "${ATTACHMENT_NAME}" attachment`)
        }
      ...
      for (let attempt = 0; ; attempt++) {
        ...
        try {
          await db.put({
            ...
            ...(rev ? { _rev: rev } : {}),
            _attachments: {
              [ATTACHMENT_NAME]: { content_type: "application/octet-stream", data: toBase64(value) },
            },
          })
          return
        } catch (e) {
