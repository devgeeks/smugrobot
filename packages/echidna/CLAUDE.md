...
- Version byte is outside the MAC: an untrusted backend can flip a `0x02` body to `0x01`, denying reads (`NEEDS_MIGRATION`) until re-migrated, but `migrate()` detects the flip and repairs such a body in place rather than laundering it into a corrupted, MAC-valid `0x02` blob.
