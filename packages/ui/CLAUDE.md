...
- `vault-input` — `{ value: string }` — fires on every keystroke (or `{ value: number }` for `vault-slider`, while dragging)
- `vault-change` — `{ value: string }` or `{ checked: boolean }` — fires on commit/blur (or `{ value: number }` for `vault-slider` on release, or `{ value: string[] }` for `vault-tag-select` on every add/remove)
